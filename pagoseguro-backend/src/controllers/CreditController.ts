...
export class CreditController {
  ...
  async listCredits(req: Request, res: Response) {
  ...
  async updateCredit(req: Request, res: Response) {
    ...
    if (!credit) return res.status(404).json({ success: false, message: 'Crédito no encontrado' });
  ...
  async deleteCredit(req: Request, res: Response) {
    const user = (req as any).user;
    if (user.role !== 'GERENTE') {
